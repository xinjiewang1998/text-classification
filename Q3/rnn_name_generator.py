...
def seqs_to_ids(seqs, char_to_id, max_len=20):
    ...
    for name in seqs:
        ...
        all_seqs.append(ids)
    # Create a result array with all zero
    result = np.zeros((len(all_seqs), max_len))
    for i, name in enumerate(all_seqs):
        if len(name) >= max_len:
            # if longer than the max length cut
            result[i] += name[:max_len]

        else:
            result[i, :len(name)] += name
    return result
...
def train_model(model, Xtrain, Ytrain, Xval, Yval, id_to_char, max_epoch):
    ...
    for e in range(max_epoch):
        for batch in range(num_batches):
            # training loop of the RNNLM model
            ...
            out, h = model(Xbatch)
            optim.zero_grad()
            # Calculating loss switch the dim 1 and 2
            loss = lossfn(out.permute(0, 2, 1), Ybatch)
            # Calculate gradients and update parameters
            loss.backward()
            optim.step()
        # print out validation loss after an epoch.
...
def gen_string(model, id_to_char, max_len=20, sample=True):
    ...
    for i in range(max_len):
        ...
        # calculate prob and using softmax
        out_softmax = nn.functional.softmax(out, 2)
        # print(out_softmax)
        # print(out_softmax.shape)
        # random sample or not
        # print(out_softmax.data.numpy().shape)
        if sample:
            id = np.random.choice(range(0, len(id_to_char)), p=out_softmax.data.numpy().ravel())
        else:
            id = torch.argmax(out_softmax)
        # print(idx)
        id = id.item()
        # print(id)
        # print(type(id))
        ...
        out_str += ch
        # the input variables for the next iteration reshape the x to (1,1)
        x = torch.tensor(np.array(id).reshape(1, 1))
        h = h_last
